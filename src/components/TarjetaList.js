 ...
 import { getUsers, deleteUser } from "../Api/ApiChofer";
 import {useIsFocused} from '@react-navigation/native' //

import ChoferItem from './ChoferItem';
import { AuthContext } from '../context/AuthContext';
import TarjetaItem from './TarjetaItem';

 const TarjetaList = ({navigation}) => {
  ...
  const {TarjetaRecorrido, setTarjetaRec, ida, setGIda} = useContext(AuthContext)
  ...
  const handleDelete=async(id)=>{
    ...
    // if(user.tipo_recorrido==true){

    // }else if(user.tipo_recorrido==true){
  ...
  const renderItem = ({item}) =>{//que sera la funcion que se ejecutara por cada usuario, y por cada item que recorra, devolverá el texto que se le indica
      ...
      // console.log({item})
        return <TarjetaItem user={item} handleDelete={handleDelete. navigation} />
  }
